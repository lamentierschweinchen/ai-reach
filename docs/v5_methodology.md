...
**Version:** 5.0
**Published:** 2026-04-19 (historical backfill 2026-04-20; reconciliation revision 2026-09-19)
...
Parameter choices were informed by the 7 historical cases and a 28-occupation cross-section from the Anthropic Economic Index (Massenkoff & McCrory, March 2026). The cross-section measures observed task *coverage* by Claude — a usage proxy that includes augmentation, not observed job loss. Against it, a single-slope fit on replaceability gives in-sample R² = 0.61 (0.56 through the origin); stratifying by barrier type raises it to 0.78 (centred R² throughout). These are in-sample fits on 28 points, not out-of-sample validation of employment effects. The [archived build worksheet](fit_worksheet_2026-04-17.csv) uses dossier-transcribed coverage estimates, approximate occupation crosswalks and earlier Phase 5 scores; its source mappings have not all been verified against primary occupation-level data. Its 0.78 is reconstructed using worksheet multipliers (NONE 0.82, HUMAN_PREFERENCE 0.47, REGULATORY 0.38, HUMANOID_DEPENDENT 0.05), not the displacement multipliers above. These statistics establish the worksheet’s arithmetic, not the validity of the current model or its employment projections. *Corrected 2026-09-19: this document previously reported "0.85+" and attributed it to the historical cases; that figure does not reproduce from the archived fit table, and the statistic belongs to the cross-section.*
...
P_A exceeds C_R in every year from 1970 through 2025; C_R first exceeds P_A in 2026. C_G rises above S_E and Phi_S in the 2020s. The remaining vectors do not follow one fixed ordering throughout the historical period.

**Crossover function.** Every historical and projected year uses the same logistic crossover — `1 / (1 + exp(-8 · (capability − difficulty)))`. At equal capability and difficulty the task receives 0.5 credit, with a continuous transition on either side. Occupation scores average these credits using estimated task-time weights. Credit is a modeling convention, not a measured fraction of time saved. *Corrected 2026-09-19: the earlier description incorrectly claimed near-term projections used a strict threshold.*

**Territory aggregation.** For each year, territory replaceability is the employment-weighted mean of in-territory occupation replaceabilities, using current (2026) employment weights within territory. This is an editorial decision: we don't have occupation-level employment weights back to 1970 at the 480-occupation granularity (ILOSTAT began in 1991), and reconstructing historical occupation weights would be a separate research project. The current framing is: "what share of today's labor composition, within each territory, would have been technically replaceable at each historical year's capability values?"
...
4. **Phase 6 barrier-model form.** Multiplicative barrier multipliers work across most of the distribution but can understate regulatory frictions because they discount only the 2026 backlog, while the capability-gain term uses a common conversion rate for every barrier. A floor-adjusted barrier model for regulatory frictions is a likely v6 change; in v5.0, these cases sit in the editorial-overrides queue.
...
The September 19 reconciliation is cited as **5.0-rev.20260919**; the scientific model remains v5.0. The dataset carries the citation identifier in `metadata.citation_version`. The homepage retains raw annual workforce shares, flags known breaks beside the timeline, and holds both shares and total workforce at the 2025 endpoint for subsequent years. Historical macro shapes aggregate children as they emerge; this is a display partition, not a new labor estimate.
