...
- **Homepage drew 1990 workforce shares for every later year.** The loader's record-type filter predates v5, which stores 1991–2025 labor records as `derived_from_occupation`; none were admitted, so each territory held its 1990 share from 1991 to 2041 (Land & Sea at 43.0% in 2026 against 26.6% in the data; header average 30 rather than 34). Live since the April 19 release. The two `modelled` armed-forces components (1988–89) were also being read as whole-year totals, so the world workforce displayed 24M for those years. Both fixed; for 2026–2041 the page holds the 2025 shares constant, because v5 does not project workforce shares.
- **Land & Sea workforce share, 1800 / 1830 / 1860.** Stored as 100.0 while Industry and Services carried their true shares (sum ≈ 121%). Normalised to the records' own worker counts: 79.1 / 77.4 / 76.1. The other 48 years were already consistent.
...
- **Mirror deep links.** `?job=` accepted only an ISCO code and fell back to Nurse on anything else — including the name form `llms.txt` documented. It now resolves an exact job name or an unambiguous alias, normalises the URL to the code, and opens the search on anything it cannot place instead of guessing.
- **Release check.** `scripts/check_release.py` derives each of these figures from the dataset and fails the deploy on any mismatch. Run against the previous release it reports 27 failures; against this one, none.
