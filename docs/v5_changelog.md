...
| HUMAN_PREFERENCE | (implicit 1.0) | **0.562 ×** (Phase 11 strengthened from Phase 6's 0.70) — *[corrected 2026-09-19: never applied; the shipped value is 0.70, see the September 19 entry]* |
| REGULATORY | (implicit 1.0) | **0.405 ×** (Phase 11 strengthened from Phase 6's 0.50) — *[corrected 2026-09-19: never applied; the shipped value is 0.50]* |
| HUMANOID_DEPENDENT | (implicit 1.0) | **0.15 ×** |

Barrier-type stratification was validated empirically at Phase 6: cross-section R² jumps from 0.61 (plain conversion × lag) to 0.85+ when barrier type is included as a categorical predictor *[corrected 2026-09-19: the archived table reproduces 0.78, not 0.85+]*. The Phase 11 recalibration strengthened HUMAN_PREFERENCE and REGULATORY proportionally to preserve empirical fit against the Anthropic Economic Index observed-coverage anchor after Phase 5 r26 scores were rescored to pure task-math. Historical-case anchors (telephone operators 1920–2023, bank tellers, travel agents, cashiers, translators, CSRs, bookkeeping clerks) constrain the lag schedule.
...
### September 19, 2026 — documentation reconciled to the data; homepage workforce shares fixed
An external audit checked the published documents against the dataset, the build scripts and the live site; every claim was then re-verified independently before anything changed. **No replaceability or displacement score changed.**

- **Homepage drew 1990 workforce shares for every later year.** The loader's record-type filter predates v5, which stores 1991–2025 labor records as `derived_from_occupation`; none were admitted, so each territory held its 1990 share from 1991 to 2041 (Land & Sea at 43.0% in 2026 against 26.6% in the data; header average 30 rather than 34). Live since the April 19 release. The two `modelled` armed-forces components (1988–89) were also being read as whole-year totals, so the world workforce displayed 24M for those years and interpolated down towards it across 1981–1989. Both fixed; for 2026–2041 the page holds the 2025 shares constant, because v5 does not project workforce shares.
- **Land & Sea workforce share, 1800 / 1830 / 1860.** Stored as 100.0 while Industry and Services carried their true shares (sum ≈ 121%). Normalised to the records' own worker counts: 79.1 / 77.4 / 76.1. In every other year the shares sum to 100 and agree with their worker counts — which is not the same as the series being smooth; see the next item.
- **Known breaks in the workforce-share series, now visible.** With the homepage reading 1991–2025 for the first time, the annual series shows classification breaks — the largest at 2010→2011 (Land & Sea −7.0 points, about 202M workers; Maintaining & Fixing +4.8), with roughly ±2-point year-to-year noise afterwards — and the 1950 cross-section is an outlier (Land & Sea 25% between 54% and 48%). These are artefacts of data assembly, not events; they are disclosed in the methodology's limitations and queued for v6. The 2025 shares that weight the headline figures rest on 19 reporting countries, with Land & Sea tied to the ILO's world agriculture total, as in 1991–2010.
- **Replacement formula: four descriptions, one calculation.** All 2,400 stored displacement values (480 occupations × 5 checkpoints) reproduce within 0.1 from the Phase 6 parameters the build scripts load — barrier multipliers NONE 1.20, ECONOMIC 1.00, HUMAN_PREFERENCE 0.70, REGULATORY 0.50, HUMANOID_DEPENDENT 0.15, applied to the lag term only. `metadata.replacement_formula_v5` advertised Phase 11's proposed 0.562 / 0.405, which never reached the data because the later recompute read the Phase 6 file; the methodology page printed a third multiplier set and a different equation, and the Markdown a fourth variant. Metadata, HTML and Markdown now state the shipped calculation. The shipped values are canonical for v5. Whether to adopt the Phase 11 proposal is recorded as an open question for the v6 re-fit: it would change 161 occupations at 2041 by up to 4.8 points and move the mean from 24.2 to 23.6.
- **Fit statistic restated.** "R² 0.85+" was attributed to the seven historical cases. The statistic belongs to the 28-occupation Anthropic Economic Index cross-section — observed task coverage, a usage proxy that includes augmentation, not job loss — and the archived fit table reproduces 0.61 (single slope), 0.56 (through the origin) and 0.78 (barrier-stratified), not 0.86. The earlier entry above that reports the jump "from 0.61 to 0.85+" should be read with this correction.
- **Capability table.** The 1985 and 2025 rows on the methodology page matched an older trajectory; replaced with the canonical values (2025 became stale when it was anchored on August 15).
- **August 17 entry, corrected figure.** "Employment-weighted mean 2041 replaceability 84.2" weighted by within-territory employment only. Weighted across the world workforce (territory share × in-territory weight, the definition the essay uses) it is **84.8**; the like-for-like gap to modeled displacement is 61.4 points weighted, 60.8 unweighted.
- **ISCO 9112** (cleaners in offices, hotels and other establishments) was labelled "Vehicle Cleaners", colliding with 9122. Now "Commercial Cleaners"; all 480 display names are unique.
- **Below-10 examples** on the methodology page named Surgeons (29.5) and Child Care Workers (18.6); now roofers, plumbers and bricklayers, which do score below 10.
- **Documentation counts.** README dataset table (tasks 4,811, sources 6,704, events 94 / 1769–2026, labor shares 1800–2025, 3–14 tasks per occupation, the actual composition of the 480 codes); `/about` task totals; thirteen territories stated consistently, with the two pre-1870 aggregates named as such and `metadata.historical_aggregate_count` added.
- **`CITATION.cff`** declared the dataset MIT (it is CC BY 4.0; MIT covers the code) and had failed CFF 1.2.0 validation since April because of an invalid reference type. Both fixed; the file validates.
- **Mirror deep links.** `?job=` accepted only an ISCO code and fell back to Nurse on anything else — including the name form `llms.txt` documented. It now resolves an exact job name or an unambiguous alias (codes in any case; names with or without hyphens), normalises the URL to the code, and opens the search on anything it cannot place instead of guessing. The share-card endpoint resolves codes and exact display names (alias links receive a generic card until the client normalises the URL) and no longer renders "undefined" for values such as `?job=constructor`.
- **Homepage and `/about` descriptions** said "2.4 billion workers", a figure that matches nothing in v5; the 2025 workforce in the data, and the page's own header, is 3.6 billion.
- **Release check.** `scripts/check_release.py` derives these figures from the dataset and compares them with the documents, the Mirror bundle, the public extracts, the share-card manifest and the homepage loader. It is a required manual step before each deploy — nothing runs it automatically. An independent review of its first version found 21 ways it could pass while something was wrong; the hardened version catches all of those cases. The final release review added a copy of the archived fit worksheet so its arithmetic can also be re-derived; this does not authenticate the worksheet’s source mappings. Browser checks remain required.


### September 19, 2026 — final release review
- **Historical rendering:** Industry and Services now aggregate the children still within them, bridging 1860 macro observations to the 1870 territory series. During the existing six-year visual transitions, children progressively take their share of the macro area; a child's label still reports its full underlying workforce. Historical territory panels now use the selected year, rather than substituting 1950. The headline average includes all 13 territories even before their individual shapes appear. Pre-1970 and macro panels show an unavailable score as a dash.
- **Known data limitations:** the 1950 outlier, the 1991–2025 breaks and the constant post-2025 workforce are documented in the methodology's known limitations. The homepage draws workforce shares at decade points plus the 2025 endpoint and skips the 1950 cross-section, so assembly breaks do not play as events; this is a display cadence. Annual records, scores and weights were not smoothed or rescored. These estimates are unsuitable for interpreting individual annual jumps as measured job losses; repairing the series remains v6 work.
- **Current method wording:** both historical and forward scoring use logistic k=8, not a strict threshold. Task credits and time weights are estimates, not measured time saved. Worksheet fit statistics use earlier Phase 5 scores and separate descriptive multipliers, not the shipped displacement model; a reproducible worksheet extract is linked beside the qualification. The overbroad historical vector ordering was replaced by statements the dated trajectories support.
- **Essay:** following the owner's clarification that the dek describes a conceptual contrast between knowledge and physical work, the dek now says machines are moving toward mastery of knowledge work while much physical work still escapes them. The body text is unchanged. Source and generated editions were rebuilt.
- **Deep-link edge cases:** overlong queries are no longer truncated into a different valid OG name. Choosing a result preserves unrelated query parameters and the fragment. The search popup now sits above the scroll hint.
- **Citation revision:** `5.0-rev.20260919` distinguishes this correction from the original v5.0; the scientific model version remains 5.0. Dataset metadata carries the citation identifier, and the public derivatives were rebuilt from it.
- **Share images:** the homepage link preview was a capture from before v5 (57% average, 3.4B workers). It now shows the 2026 view (34%, 3.6B) and its alt text names 2026. The generic Mirror card now shows the wall of 480 occupations from the essay, with the current project name. Image URLs are versioned so cached previews refresh.
- **Verification:** release checks now compare all CSV cells and all occupation-index fields, bundle task inputs and score fields, execute historical share-partition tests, and reconstruct the archived worksheet's R². They check release invariants, not scientific validity or every statement on the site.
