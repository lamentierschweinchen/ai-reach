...
- **Mirror deep links.** `?job=` accepted only an ISCO code and fell back to Nurse on anything else — including the name form `llms.txt` documented. It now resolves an exact job name or an unambiguous alias (codes in any case; names with or without hyphens), normalises the URL to the code, and opens the search on anything it cannot place instead of guessing. The share-card endpoint resolves names the same way and no longer renders "undefined" for values such as `?job=constructor`.
